...
optional model. Omit `profile.model` to inherit the current Pi model; see
[model-selection.md](../skills/pi-orchestra/references/model-selection.md)
before overriding it. The runtime creates a bus subscription for each spawned
subagent; `AgentRun.busId` remains the lifecycle/query scope, not the context
delivery path.
